...
@Component({
  selector: 'app-rdap-manage-pin-list',
  templateUrl: './rdap-manage-pin-list.component.html',
  styleUrls: ['./rdap-manage-pin-list.component.scss']
})
export class RdapManagePinListComponent implements OnInit {
  ...
  constructor(private httpClient: HttpClient, location: Location, private _router: Router,
    ...
    this.ddlUrl = environment.baseapiurl;
    this.baseApi = environment.extrapinreqapiurl;
    this.exportfilename = "managepin_requested_data";
    this.perPage=10;
    this.pageNumber = 0;
    this.searchUrl = this.baseApi + "managepin/search";
  ...
  serchfilterFormGroup() {
    this.filterform = this.fb.group({
      planitem: null,
      planitemsearchcriteria: "Equal",
      channel: null,
      channelsearchcriteria: "Contains",
      channeltype: null,
      channeltypesearchcriteria: "Contains",
      market: null,
      marketsearchcriteria: "Contains",
      region: null,
      regionsearchcriteria: "Contains"
    });
  }
  searchSubmit() {
    this.spinner.show();
    this.searchParam = { pageNumber: 1, pageSize: this.perPage, filters: [], sorts: [] };
    this.filterparamarr = [];
    if(this.filterform.value["channel"]!=null && !isEmptyString(this.filterform.value["channel"])){
      this.filterparamarr.push({field:"channel",operator:this.filterform.value["channelsearchcriteria"],value:this.filterform.value["channel"]})
    }
    if(this.filterform.value["planitem"]!=null && !isEmptyString(this.filterform.value["planitem"])){
      this.filterparamarr.push({field:"planitem",operator:this.filterform.value["planitemsearchcriteria"],value:this.filterform.value["planitem"]})
    }
    if(this.filterform.value["channeltype"]!=null && !isEmptyString(this.filterform.value["channeltype"])){
      this.filterparamarr.push({field:"channeltype",operator:this.filterform.value["channeltypesearchcriteria"],value:this.filterform.value["channeltype"]})
    }
    if(this.filterform.value["market"]!=null && !isEmptyString(this.filterform.value["market"])){
      this.filterparamarr.push({field:"market",operator:this.filterform.value["marketsearchcriteria"],value:this.filterform.value["market"]})
    }
    if(this.filterform.value["region"]!=null && !isEmptyString(this.filterform.value["region"])){
      this.filterparamarr.push({field:"region",operator:this.filterform.value["regionsearchcriteria"],value:this.filterform.value["region"]})
    }
    this.searchParam.filters = this.filterparamarr;
    this.searchParam.sorts.push({ field: "Planitem", direction: "DESC" });
    let sortparamarr = [];
    sortparamarr.push({ field: "Planitem", direction: "DESC" });
    this.searchGridData = [];
  ...
  callDdlApi() {
    ...
    this.regionarrobj = [];
    this.masterApiService.masterSearchDDL(this.ddlUrl + "version/ddl").subscribe(data => {
      this.versionarrobj.push(data);
    });
    this.masterApiService.masterSearchDDL(this.ddlUrl + "channel/ddl").subscribe(data => {
      this.channelarrobj.push(data);
    });
    this.masterApiService.masterSearchDDL(this.ddlUrl + "channeltype/ddl").subscribe(data => {
      this.channeltypearrobj.push(data);
    });
    this.masterApiService.masterSearchDDL(this.ddlUrl + "gamecomplexity/ddl").subscribe(data => {
      this.gamecomplexityarrobj.push(data);
    });
    this.masterApiService.masterSearchDDL(this.ddlUrl + "region/ddl").subscribe(data => {
